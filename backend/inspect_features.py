...
def main():
    # Fișierele folosite și în antrenare
    ...
    # Asigurăm numele corect pentru label
    df_atacuri = df_atacuri.rename(columns={" atac": "atac"})

    # Merge pe regiune, ca în antrenare
    df = pd.merge(
        df_atacuri,
        df_strategic,
    ...
    for col in numeric_cols:
        if col not in df.columns:
            continue
        x = df[col]
        # Sărim coloanele cu valori constante
        if x.nunique() <= 1:
            continue
