...
def evaluate_config(df, w_dist, w_infra, w_front, w_nuclear, w_night, w_weather, quantile):
    ...
    # Feature-uri numerice + encode simplu pentru categorice
...
def main():
    ...
    # grilă FOARTE mică de valori pentru un „sweep” rapid (doar câteva combinații)
    quantiles = [0.70]              # doar 30% risc_inalt
    w_dists = [0.5, 1.0]            # distanța față de front
    w_infras = [0.5, 1.0]           # importanța infrastructurii
    w_fronts = [0.5, 1.0]           # distanța strategică față de front
    w_nuclears = [1.5, 2.0]         # prezență nucleară
    w_nights = [1.0, 2.0]           # noapte
    w_weathers = [1.0, 1.5]         # vreme bună
